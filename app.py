...
class QuizApp:
    ...
    def filter_data(self):
        ...
        # カテゴリが変更された場合、分野の選択を「すべて」にリセット
        if new_category != current_category:
            st.session_state.filter_field = "すべて"
            current_field = "すべて" # 現在のfieldも更新して次のselectboxに反映させる
            st.rerun() # カテゴリ変更時に再描画を強制し、分野の選択肢をリセット

        # 新しいカテゴリに基づいて分野の選択肢を絞り込む
        if new_category != "すべて":
            filtered_df_for_field = st.session_state.quiz_df[st.session_state.quiz_df["カテゴリ"] == new_category]
        ...
        # 現在の分野が新しい選択肢に存在しない場合、'すべて'にリセット (念のため)
        ...
        # 試験区分の選択肢を生成し、selectboxを表示
        level_options = ["すべて"] + sorted
