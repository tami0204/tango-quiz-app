...
class QuizApp:
    def __init__(self, df: pd.DataFrame):
        ...
        if st.session_state.quiz_df is None:
            ...
            # '〇×結果' 列が存在するか確認し、なければ追加
            if '〇×結果' not in st.session_state.quiz_df.columns:
                st.session_state.quiz_df['〇×結果'] = '' # 新しい列を空文字列で初期化
            else:
            ...
            # (任意) 初期読み込み時に'〇×結果'列を元に正解回数/不正解回数を設定する場合
            # 例: 初期の'〇×結果'が'〇'なら正解回数を1に、'×'なら不正解回数を1に
            # for index, row in st.session_state.quiz_df.iterrows():
            #     if row['〇×結果'] == '〇':
            #         st.session_state.quiz_df.at[index, '正解回数'] = 1
            #     elif row['〇×結果'] == '×':
            #         st.session_state.quiz_df.at[index, '不正解回数'] = 1
                
        self.initial_df = df.copy() # セッションリセット用に元のDataFrameを保持
    ...
    def _reset_session_state(self):
        ...
        # (任意) 初期読み込み時と同様に'〇×結果'列を元に正解回数/不正解回数を再設定する場合
        # for index, row in st.session_state.quiz_df.iterrows():
        #     if row['〇×結果'] == '〇':
        #         st.session_state.quiz_df.at[index, '正解回数'] = 1
        #     elif row['〇×結果'] == '×':
    ...
    def _handle_answer_submission(self, selected_option_text: str, current_quiz_data: dict):
        ...
        if word in temp_df["単語"].values:
            ...
            # --- 〇×結果列と正解・不正解回数の更新ロジック ---
            temp_df.at[idx, '〇×結果'] = result_mark # 〇×結果列に最新の〇または×を上書き
...
try:
    ...
    # 必要な列がCSVに存在するかチェック (今回は'〇×結果'を含む)
